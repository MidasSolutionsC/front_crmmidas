...
import { TypeUserPermissionList } from 'src/app/core/models/api/settings/type-user-permission.model';
@Injectable({
  providedIn: 'root'
})
export class TypeUserPermissionService {
  private cachedData: ResponseApi; // Almacena los datos en caché
  private listSubject: BehaviorSubject<TypeUserPermissionList[]> = new BehaviorSubject<TypeUserPermissionList[]>([]);
  public listObserver$: Observable<TypeUserPermissionList[]> = this.listSubject.asObservable();
  ...
  private get baseUrl(){
    return this.configService.apiUrl + 'typeUserPermission';
  }
  ...
  addObjectObserver(typeUserList: TypeUserPermissionList) {
  ...
  addArrayObserver(typeUserPermissionList: TypeUserPermissionList[]) {
    this.listSubject.next(typeUserPermissionList);
  }

  // Método para modificar un objeto en el array
  updateObjectObserver(typeUserPermissionList: TypeUserPermissionList) {
    const currentData = this.listSubject.getValue();
    const index = currentData.findIndex(item => item.id === typeUserPermissionList.id);
    if (index !== -1) {
      currentData[index] = typeUserPermissionList;
      this.listSubject.next(currentData);
    }
  }
