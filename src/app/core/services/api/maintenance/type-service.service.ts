...
import { ResponseApi, TypeServiceList } from '../../../models';
...
export class TypeServiceService {
  ...
  private listSubject: BehaviorSubject<TypeServiceList[]> = new BehaviorSubject<TypeServiceList[]>([]);
  public listObserver$: Observable<TypeServiceList[]> = this.listSubject.asObservable();
  ...
  private get baseUrl(){
    return this.configService.apiUrl + 'typeService';
  }
  ...
  addObjectObserver(typeServiceList: TypeServiceList) {
    const currentData = this.listSubject.getValue();
    currentData.push(typeServiceList);
    this.listSubject.next(currentData);
  }

  // Método para actualizar todo el array
  addArrayObserver(typeServiceList: TypeServiceList[]) {
    this.listSubject.next(typeServiceList);
  }

  // Método para modificar un objeto en el array
  updateObjectObserver(typeServiceList: TypeServiceList) {
    const currentData = this.listSubject.getValue();
    const index = currentData.findIndex(item => item.id === typeServiceList.id);
    if (index !== -1) {
      currentData[index] = typeServiceList;
      this.listSubject.next(currentData);
    }
  }
