...
import { TypeCurrencyList, ResponseApi } from 'src/app/core/models';
...
export class TypeCurrencyService {
  ...
  private listSubject: BehaviorSubject<TypeCurrencyList[]> = new BehaviorSubject<TypeCurrencyList[]>([]);
  public listObserver$: Observable<TypeCurrencyList[]> = this.listSubject.asObservable();
  ...
  private get baseUrl(){
    return this.configService.apiUrl + 'typeCurrency';
  }
  ...
  addObjectObserver(typeCurrencyList: TypeCurrencyList) {
    const currentData = this.listSubject.getValue();
    currentData.push(typeCurrencyList);
    this.listSubject.next(currentData);
  }

  // Método para actualizar todo el array
  addArrayObserver(typeCurrencyList: TypeCurrencyList[]) {
    this.listSubject.next(typeCurrencyList);
  }

  // Método para modificar un objeto en el array
  updateObjectObserver(typeCurrencyList: TypeCurrencyList) {
    const currentData = this.listSubject.getValue();
    const index = currentData.findIndex(item => item.id === typeCurrencyList.id);
    if (index !== -1) {
      currentData[index] = typeCurrencyList;
      this.listSubject.next(currentData);
    }
  }
