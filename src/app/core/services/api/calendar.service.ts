...
import { CalendarList, ResponseApi } from '../../models';
...
export class CalendarService {
  ...
  private listSubject: BehaviorSubject<CalendarList[]> = new BehaviorSubject<CalendarList[]>([]);
  public listObserver$: Observable<CalendarList[]> = this.listSubject.asObservable();
  ...
  private get baseUrl(){
    return this.configService.apiUrl + 'calendar';
  }
  ...
  addObjectObserver(calendarList: CalendarList) {
    const currentData = this.listSubject.getValue();
    currentData.push(calendarList);
    this.listSubject.next(currentData);
  }

  // Método para actualizar todo el array
  addArrayObserver(calendarList: CalendarList[]) {
    this.listSubject.next(calendarList);
  }

  // Método para modificar un objeto en el array
  updateObjectObserver(calendarList: CalendarList) {
    const currentData = this.listSubject.getValue();
    const index = currentData.findIndex(item => item.id === calendarList.id);
    if (index !== -1) {
      currentData[index] = calendarList;
      this.listSubject.next(currentData);
    }
  }
